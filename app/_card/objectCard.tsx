...
// montage, on-the-fly color cutout, ResultCard). Imported by BOTH the Query/Search
// page and the Explore/Map page so a source looks identical however you reach it.
// Extracted from the original app/search/page.tsx — behavior is unchanged, except the
// static RGB PNG is replaced by the on-the-fly <FitsglCutout>, and "flag as spurious"
// files into the shared Supabase queue.
import { useState, type ReactNode, type CSSProperties } from "react";
import dynamic from "next/dynamic";

// On-the-fly WebGL color cutout (window + WebGL2), loaded client-only via next/dynamic
// with { ssr: false } — required by this static export, same pattern as the /map viewer.
// Replaces the retired pre-baked RGB PNG (RgbStamp) for CEERS (the only SPAM field).
...
// cutout). Kept in sync with FITSGL_BASE in FitsglCutout.tsx. SPAM is CEERS-only.
const FITSGL_FIELDS = new Set(["SPAM", "CEERS"]);
...
// ---- named-object labels ---------------------------------------------------
// A small curated list of famous objects (Maisie's Galaxy, CEERS 1019, …), resolved to
// the field's current id by position (scripts/make_labels.py -> searchindex/labels.json).
...
// the color on the fly from the field's fitsgl tiles (same WebGL color as /map).
// Kept only as a reference/fallback; ResultCard no longer uses it.
...
export const CORRAL_DEFAULT = "https://web.corral.tacc.utexas.edu/unicorn/spam";
// The search index + zgrid are served from the site itself (GitHub Pages / Fastly
// CDN) — fast + edge-cached for everyone — while the per-object files stay on Corral.
export const INDEX_BASE = "/spam/searchindex";
...
// Fields with a web search index live on Corral. SPAM is single-field (CEERS).
export const SEARCH_FIELDS: { field: string; dir: string; prefix: string; version: string; available: boolean }[] = [
  { field: "SPAM", dir: "", prefix: "ceers", version: VERSION, available: true },   // data flat under Corral .../unicorn/spam/
];
...
export const webBase = (baseRoot: string, dir: string) => (dir ? `${baseRoot}/${dir}/web` : `${baseRoot}/web`);
...
// Fetch a JSON file, transparently handling a gzipped (.json.gz) sibling. The large
// search index is served gzipped (~8x smaller); prefer the .gz and decompress in-browser.
...
export async function loadField(fc: FieldConfig): Promise<{ idx: FieldIndex; zg: ZGrid }> {
  // Dedupe concurrent loads onto one in-flight fetch.
...
export async function loadFilters(fc: FieldConfig): Promise<Record<string, NumCol> | null> {
  ...
  _filtersPromise[fc.field] = (async () => {
    ...
    try {
      ...
      if (override) throw e;
      return await fetchJsonMaybeGz(`${webBase(CORRAL_DEFAULT, fc.dir)}/${name}`);
    }
  })();
  try {
...
export async function loadSpecz(fc: FieldConfig): Promise<Record<string, SpeczRec>> {
  ...
  _speczPromise[fc.field] = (async () => {
    ...
    try {
      ...
      if (!override) {
        try {
          const d = await fetchJsonMaybeGz(`${webBase(CORRAL_DEFAULT, fc.dir)}/${name}`);
          return (d && d.objects) || {};
        } catch { /* fall through */ }
      }
      return {};
...
// Served from the site CDN (INDEX_BASE = /spam/searchindex), with a Corral fallback if
// the site copy is missing — mirrors loadSpecz's ?data= override / fallback pattern.
...
export async function loadLabels(): Promise<LabelRec[]> {
  ...
  _labelsPromise = (async () => {
    const override = dataOverride();
    const primary = override ? webBase(override, SEARCH_FIELDS[0].dir) : INDEX_BASE;
    const parse = (data: any): LabelRec[] => (Array.isArray(data) ? data : data?.labels ?? []) as LabelRec[];
    try {
      return parse(await fetchJsonMaybeGz(`${primary}/labels.json`));
    } catch {
      if (!override) {
        try {
          return parse(await fetchJsonMaybeGz(`${webBase(CORRAL_DEFAULT, SEARCH_FIELDS[0].dir)}/labels.json`));
        } catch { /* fall through */ }
      }
      return [];
    }
...
// The loaded index for a field, if it has been fetched this session.
...
export async function fetchObject(fc: FieldConfig, id: number, zg: ZGrid): Promise<SourceResult | null> {
  try {
    ...
    return {
      ...
      rgbUrl: `${webBase(corralBase(), fc.dir)}/rgb/${fc.prefix}_${id}.png`,
      selFail,
...
// Instrument / marker colors, shared with the legend (SPAM palette).
const HST_COLOR = "#6B4FD6";    // HST/ACS detections (SPAM wl-1 purple)
const JWST_COLOR = "#2F7DD1";   // JWST/NIRCam detections (SPAM wl-2 blue)
const MODEL_COLOR = "#D89A1E";  // best-fit model (SPAM wl-4 gold)
const LOWZ_COLOR = "#C4452F";   // low-z (z<7) alternative model (SPAM wl-5 red)

// Inline SED plot (SVG), log flux axis. Circle size encodes bandwidth (large = wide,
// small = medium/narrow); color encodes instrument. The reconstructed best-fit (gold)
// and low-z (red dashed) model spectra overlay the points.
...
// P(z) plot — fiducial (solid) with the low-z (z<7) alternative overlaid (dashed red).
export function PZPlot({ zgrid, pz, za, zgridLowz, pzLowz }: {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${w} ${h}`} style={{ maxWidth: w, display: "block" }}>
      <polygon points={fill} fill="rgba(47,125,209,0.12)" />
      {lowzOk && <polyline points={lowzPts} fill="none" stroke={LOWZ_COLOR} strokeWidth={1.5} strokeDasharray="4,3"/>}
      <polyline points={pts} fill="none" stroke="#2F7DD1" strokeWidth={1.8}/>
  ...
  return (
    <div className="card-bright" style={{ padding: "1.25rem", marginBottom: "12px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem", flexWrap: "wrap", gap: "8px" }}>
        <div>
          {/* Famous-object label (e.g. "Maisie's Galaxy") — solid accent pill, white
              text, legible on the light theme (SPAM has no lavender/pink pastels). */}
          {src.nickname && (
            <div className="mono" style={{ marginBottom: "6px", display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
              <span title={[src.nickname.ref, src.nickname.z != null ? `z=${src.nickname.z}` : "", src.nickname.note].filter(Boolean).join(" · ")}
                style={{
                  ...
                  background: "linear-gradient(135deg, var(--accent), var(--accent3))", color: "#ffffff", letterSpacing: "0.02em",
                }}>
                ★ {src.nickname.name}
              </span>
      ...
      {/* Plots — SED + P(z) wrapped in data-plot markers so the download grabs each
          plot's MAIN svg (not the SED legend's glyph svgs). */}
      ...
      {/* On-the-fly color cutout — rendered live from the field's fitsgl tiles (same
          WebGL trilogy color as /map), NOT a pre-baked PNG. */}
...
// Flag a source as spurious: file it into the SAME Supabase `flags` queue UNICORN uses
// (status 'pending') for triage. No email. Anonymous callers can insert only (RLS).

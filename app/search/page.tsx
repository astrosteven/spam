...
// ---- data wiring -----------------------------------------------------------
const VERSION = "0.98";
const CORRAL_DEFAULT = "https://web.corral.tacc.utexas.edu/unicorn/spam";
...
const INDEX_BASE = "/spam/searchindex";
...
async function loadFilters(fc: typeof SEARCH_FIELDS[0]): Promise<Record<string, NumCol> | null> {
  ...
  _filtersPromise[fc.field] = (async () => {
    const override = dataOverride();
    const name = `${fc.prefix}_filters_v${VERSION}.json`;
    const primary = override ? webBase(override, fc.dir) : INDEX_BASE;
    try {
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.8rem" }}>
              ...
              <tbody>
                {queryRows.map((r, i) => {
                  ...
                  {open && (
                    <tr ref={cardRef}>
                      <td colSpan={5 + queryCols.length} style={{ padding: "0.5rem 0.75rem 1rem", background: "rgba(47,125,209,0.04)" }}>

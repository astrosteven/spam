...
// support = / !=. Per-filter mag_<f>/snr_<f>/flux_<f> and colors <a>-<b> are numeric.
...
function CardPlots({ src }: { src: SourceResult }) {
  ...
  return (
    <div style={{ width: 500 }}>
      {/* data-plot wrappers so the download grabs each plot's MAIN svg, NOT the SED
          legend's glyph svgs (querySelectorAll('svg')[1] would be a legend glyph). */}
...
export default function SearchPage() {
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (src) {
        ...
        try {
          ...
          // Grab each plot's MAIN svg via its data-plot wrapper — NOT querySelectorAll('svg')[1],
          // which is one of the SED legend's glyph svgs (that bug shipped a legend glyph as the P(z)).
  ...
  function idxToSource(m: { fc: typeof SEARCH_FIELDS[0]; id: number; r: IdxRow; cz: SpeczRec | null }): SourceResult {
    ...
    return {
      ...
      row: {
        ID: m.id, RA: n(r.ra), DEC: n(r.dec), X: n(r.x), Y: n(r.y),
    ...
    <main style={{ padding: "3rem 2rem", maxWidth: "960px", margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: "2.5rem" }}>
        ...
        <p style={{ color: "var(--text-dim)", fontSize: "0.8rem", marginTop: "10px", lineHeight: 1.7 }}>
          <span style={{ color: "var(--wl-4)", fontWeight: 700 }}>Note on precision:</span> query values are rounded
          ...
          M_UV, β and sizes to 3–4 decimals. Use the FITS catalog for full-precision, science-grade values. See
          “field definitions” below for the per-quantity detail.
        </p>
      </div>
      ...
      <div className="card-bright" style={{ padding: "1.5rem", marginBottom: "2rem" }}>
        ...
        <div style={{ display: "flex", gap: "4px", marginBottom: "1.5rem" }}>
          {([
            ...
            <button key={tab.key} onClick={() => setMode(tab.key)} style={{
              ...
              background: mode === tab.key ? "rgba(47,125,209,0.12)" : "transparent",
        ...
        {mode === "upload" && (
          <div>
            ...
            <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem", fontSize: "0.8rem", color: "var(--text-dim)", fontFamily: "'JetBrains Mono', monospace" }}>
            ...
            <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
              ...
              <button onClick={() => fileRef.current?.click()} style={{
                ...
                border: "1px solid rgba(47,125,209,0.3)", borderRadius: "4px",
        ...
        {mode === "query" && (
          <div>
            ...
            <label style={{ display: "block", fontSize: "0.72rem", color: "var(--text-dim)", fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.1em", margin: "12px 0 6px" }}>
            ...
            <input
              ...
              style={{
                ...
                fontSize: "0.85rem", fontFamily: "'JetBrains Mono', monospace", outline: "none",
              }}
            />
            <div style={{ marginTop: "10px", fontSize: "0.75rem", color: "var(--text-dim)", fontFamily: "'JetBrains Mono', monospace", lineHeight: 1.9 }}>
              ...
              <button onClick={() => setDefsOpen(o => !o)} style={{ background: "none", border: "none", padding: "0 0 6px", color: "var(--accent)", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.75rem", cursor: "pointer", letterSpacing: "0.04em" }}>
              ...
              {defsOpen && (
                <div style={{ marginTop: "8px", padding: "7px 10px", background: "rgba(216,154,30,0.08)", border: "1px solid rgba(216,154,30,0.28)", borderRadius: "6px", color: "var(--text-muted)", fontSize: "0.72rem", lineHeight: 1.7 }}>
                  ...
                  mag/S/N/color match the catalog to ≲10⁻³. Redshifts, M_UV, β and sizes are stored to 3–4 decimals.
                  Colors use a 1σ upper limit for any band with S/N&lt;1. Use the FITS catalog for full-precision values.
                </div>
              )}
              <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "8px" }}>
                {[
                  ...
                  <button key={ex} onClick={() => setQueryInput(ex)} style={{
                    background: "var(--accent-dim)", color: "var(--accent)",
                    border: "1px solid rgba(47,125,209,0.25)", borderRadius: "999px",
      ...
      {/* Results */}
      {status === "searching" && (
        <div style={{ textAlign: "center", padding: "3rem", color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.85rem" }}>
          ...
          {results.map((src, i) => (
            <div key={i}>
              <div style={{ textAlign: "right", marginBottom: "-6px" }}>
                <Link href={`/map?id=${src.row["ID"]}`} className="mono" style={{ color: "var(--accent)", textDecoration: "none", fontSize: "0.72rem" }} title="Open in the Explore map">map ↗</Link>
              </div>
              <ResultCard src={src} />
            </div>
          ))}
              ...
              <tbody>
                {queryRows.map((r, i) => {
                  ...
                  <tr
                    ...
                    style={{ borderTop: "1px solid var(--border)", cursor: "pointer", background: open ? "rgba(47,125,209,0.10)" : "transparent" }}
                    onMouseEnter={e => { if (!open) e.currentTarget.style.background = "rgba(47,125,209,0.06)"; }}
                  ...
                  >
                    ...
                    {/* Open this object in the Explore map (deep-link ?id=). Stop propagation
                        so the click doesn't also toggle the row's inline bio card. */}
                    <td style={{ padding: "7px 14px", textAlign: "right", fontSize: "0.72rem" }} onClick={e => e.stopPropagation()}>
                      <Link href={`/map?id=${r.id}`} style={{ color: "var(--accent)", textDecoration: "none" }} title="Open in the Explore map">map ↗</Link>
                    </td>
                    <td style={{ padding: "7px 14px", textAlign: "right", color: "var(--accent2)", fontSize: "0.72rem" }}>{open ? "▾ close" : "view →"}</td>
                  </tr>
                  {open && (
                    <tr ref={cardRef}>
                      <td colSpan={8 + queryCols.length} style={{ padding: "0.5rem 0.75rem 1rem", background: "rgba(47,125,209,0.04)" }}>
                        {queryCard && queryCard.row["ID"] === r.id
                          ? <ResultCard src={queryCard} />
                          : <div style={{ padding: "1.5rem", textAlign: "center", color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.8rem" }}>Loading…</div>}
                      </td>
                    </tr>
                  )}
                  </Fragment>
  ...
  return (
    <div className="card" style={{ padding: "0.85rem 1.1rem", marginBottom: "1rem" }}>
      <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
        ...
        {/* Format toggle */}
        <div style={{ display: "inline-flex", border: "1px solid var(--border-bright)", borderRadius: "999px", overflow: "hidden", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.75rem" }}>
        ...
        {note && <span style={{ fontSize: "0.75rem", color: "var(--text-dim)", fontFamily: "'JetBrains Mono', monospace" }}>{note}</span>}
      </div>
      {open && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 16px", marginTop: "0.9rem", fontSize: "0.78rem", color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace" }}>
          {DL_COLS.map(c => (
...
function SearchButton({ onClick, loading }: { onClick: () => void; loading: boolean }) {
  return (
    <button onClick={onClick} disabled={loading} style={{
      background: loading ? "#e6e8ec" : "var(--accent)",

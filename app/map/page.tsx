"use client";
// Explore — an interactive fitsgl WebGL color map of CEERS. Pan/zoom the NIRCam RGB
// mosaic; every catalog source is drawn as a clickable Kron-ellipse overlay (green =
// selected, yellow = not) that opens the SAME SED / P(z) ResultCard used by the Query
// page (shared app/_card module). A sidebar filters the shown set live, and a "go to"
// box recenters on an object ID or ra,dec. SPAM is CEERS-only — no field switcher.
//
...
const MapViewer = dynamic(() => import("./MapViewer"), {
  ...
  loading: () => (
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "#0d0a1a" }}>
      <span className="mono" style={{ color: "#9aa0aa", fontSize: "0.85rem" }}>Loading viewer…</span>
    </div>
  ),
});

// The single SPAM field (CEERS). Its fitsgl tiles are the shared ones on Corral.
const FIELD: FieldConfig = SEARCH_FIELDS[0];
const FITSGL_ROOT = "https://web.corral.tacc.utexas.edu/unicorn/fitsgl";
const FITSGL_SLUG = "ceers";

// Bands offered in the magnitude-filter dropdown.
const MAG_BANDS = ["F277W", "F444W", "F090W", "F115W", "F150W", "F200W", "F356W", "F410M"];

// "Go to" recenter FOV: show a ~5" region around the target.
...
// Base URL for the field's fitsgl tiles: the ?data= mirror if set, else public Corral.
function tileBase(): string {
  if (typeof window !== "undefined") {
    const o = new URLSearchParams(window.location.search).get("data");
    if (o) return `${o.replace(/\/$/, "")}/fitsgl/${FITSGL_SLUG}`;
  }
  return `${FITSGL_ROOT}/${FITSGL_SLUG}`;
}

// Deep-link support: /map?id=<objid> jumps to the object (used by the Query page's
// "map ↗" link).
...
export default function MapPage() {
  const configUrl = useMemo(() => `${tileBase()}/fitsgl.json`, []);
  ...
  const openSource = useCallback(async (id: number) => {
    ...
    try {
      const { zg } = await loadField(FIELD);
      const src = await fetchObject(FIELD, id, zg);
      setPanel(prev => {
  ...
  // Also open the object's card once (independent of the zoom retry).
  useEffect(() => {
    if (!ready || !pendingGotoRef.current) return;
    const g0 = pendingGotoRef.current;
    void openSource(Number(g0));
    let tries = 0;
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap", marginBottom: "2px" }}>
              ...
              <span className="mono" style={{
                ...
                border: "1px solid rgba(47,125,209,0.30)", borderRadius: "5px",
                padding: "3px 10px", fontSize: "0.8rem", fontWeight: 700, letterSpacing: "0.04em",
              }}>
                CEERS
              </span>
            </div>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0, position: "relative" }}>
        ...
        {panelOpen && (
          <aside style={{
            ...
            boxShadow: "-12px 0 40px rgba(0,0,0,0.15)", zIndex: 20,
          }}>
            <div style={{
            ...
            <div style={{ padding: "1rem" }}>
              {panel.kind === "loading" && (
                <div style={{ padding: "2.5rem 1rem", textAlign: "center", color: "var(--text-muted)", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.85rem" }}>
              ...
              {panel.kind === "notfound" && (
                <div className="card" style={{
                  ...
                  background: "rgba(216,154,30,0.06)", color: "var(--text-muted)", fontSize: "0.85rem",
                }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "3px", minWidth: "260px" }}>
      <div style={{ display: "flex", gap: "6px" }}>
        <input
          ...
          style={{
            flex: 1, background: "#fff", border: "1px solid var(--border-bright)", borderRadius: "5px",
            color: "var(--text)", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.8rem", padding: "6px 9px",
          }}
        />
        <button
          onClick={() => onGo(val)}
          className="mono"
          style={{
            background: "var(--accent-dim)", border: "1px solid rgba(47,125,209,0.35)", borderRadius: "5px",
...
function FilterSidebar({
  ...
  const inputStyle: React.CSSProperties = {
    width: "100%", background: "#fff", border: "1px solid var(--border-bright)", borderRadius: "5px",
    color: "var(--text)", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.8rem", padding: "6px 8px",
  };
    ...
    <aside style={{
      ...
      <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", marginBottom: "1.1rem" }}>
        <input
          ...
          style={{ accentColor: "#1F9E9E", width: "15px", height: "15px" }}
        />
      ...
      <button
        ...
        className="mono"
        style={{
          background: "#fff", border: "1px solid var(--border-bright)", borderRadius: "5px",
